...
async def top_test(dut):
    ...
    for i in range(0, max_count, FCW_VALUE):
        ...
        assert dut.data_out.value.integer == lut_values[i], f"[{i}] Expected {lut_values[i]}, got {dut.data_out.value.integer}, value={FCW_VALUE}"
        await RisingEdge(dut.clk_in)
        samples.append(dut.pwm_out.value.integer)
